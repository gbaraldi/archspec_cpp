    ...
    if (argc > 1) {
        ...
        if (host.family() == "x86_64") {
            features = {"sse",    "sse2",    "sse3",     "ssse3",    "sse4_1",  "sse4_2", "avx",
                        "avx2",   "avx512f", "avx512vl", "avx512bw", "fma",     "bmi1",   "bmi2",
                        "popcnt", "aes",     "xsave",    "xsavec",   "xsaveopt"};
        } else if (host.family() == "aarch64") {
            features = {"neon", "fp",    "asimd",   "aes", "sha1",
                        "sha2", "crc32", "atomics", "sve", "sve2"};
        } else if (host.family() == "ppc64le" || host.family() == "ppc64") {
            features = {"altivec", "vsx", "fma"};
        } else {
